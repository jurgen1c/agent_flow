...
export const MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES = 10 * 1024 * 1024;
...
export class AgentFlowArtifactTransformError extends Error {
  ...
  constructor(message: string, code = "AGENT_FLOW_ARTIFACT_TRANSFORM", options?: ErrorOptions) {
  ...
  register(name: string, transform: AgentFlowArtifactTransform): this {
    ...
    if (this.transforms.has(normalized)) {
      throw new AgentFlowArtifactTransformError(
        ...
        "AGENT_FLOW_ARTIFACT_TRANSFORM_COLLISION"
      );
    }
...
export function executeAgentFlowArtifactTransform(
  ...
  if (transform === undefined) {
    throw new AgentFlowArtifactTransformError(
      ...
      "AGENT_FLOW_ARTIFACT_TRANSFORM_UNKNOWN"
    );
  }

  let input: ReturnType<AgentFlowRunStateStore["readArtifact"]>;
  try {
    input = store.readArtifact(runId, inputPath, { maxBytes: MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES });
  } catch (error) {
    if (error instanceof AgentFlowRunStateError) {
      const guidance = error.code === "AGENT_FLOW_ARTIFACT_TOO_LARGE"
      ...
      throw new AgentFlowArtifactTransformError(
        ...
        "AGENT_FLOW_ARTIFACT_TRANSFORM_INPUT",
        { cause: error }
      );
    }
    throw error;
  ...
  try {
    ...
    throw new AgentFlowArtifactTransformError(
      ...
      "AGENT_FLOW_ARTIFACT_TRANSFORM_FAILED",
      { cause: error }
    );
  }
...
export function transformAgentFlowFixtureArtifact(
  ...
  if (source.byteLength > MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES) {
    throw new AgentFlowArtifactTransformError(
      `Fixture artifact ${context.inputPath} exceeds the ${MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES}-byte transform input limit.`,
      "AGENT_FLOW_ARTIFACT_TRANSFORM_INPUT"
    );
  }
  ...
  return typeof result.content === "string"
    ? result.content
    : { __agent_flow_binary__: "base64", data: Buffer.from(result.content).toString("base64") };
}
...
function validateTransformOutput(transformName: string, output: AgentFlowArtifactTransformOutput): void {
  if (typeof output?.contentType !== "string" || output.contentType.trim().length === 0) {
    throw new AgentFlowArtifactTransformError(
      ...
      "AGENT_FLOW_ARTIFACT_TRANSFORM_OUTPUT"
    );
  }
}
...
function requiredName(value: unknown, label: string): string {
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new AgentFlowArtifactTransformError(`${label} must be a non-empty string.`, "AGENT_FLOW_ARTIFACT_TRANSFORM_INVALID");
  }
  return value.trim();
}
...
function isBinaryArtifactValue(value: AgentFlowYamlValue): value is AgentFlowBinaryArtifactValue {
  return isRecord(value)
    && value.__agent_flow_binary__ === "base64"
