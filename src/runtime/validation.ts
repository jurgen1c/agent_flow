...
function validateRequiredStepFields(context: StepContext, errors: AgentFlowWorkflowIssue[]): void {
  ...
  if (context.type === "command" && typeof context.step.timeout_seconds === "number" &&
      Number.isFinite(context.step.timeout_seconds) && context.step.timeout_seconds > MAX_AGENT_FLOW_COMMAND_TIMEOUT_SECONDS) {
    addStepIssue(
      errors,
      context,
      "workflow.command.timeout.invalid",
      "timeout_seconds",
      `Command timeout_seconds cannot exceed ${MAX_AGENT_FLOW_COMMAND_TIMEOUT_SECONDS}.`
    );
  }
  ...
  if ((context.type === "command" || context.type === "artifact_transform" || context.type === "session_request" || context.type === "mcp_call") && isRecord(context.step.on_failure)) {
    ...
    const failureThen = typeof context.step.on_failure.then === "string"
      ...
      : undefined;
    if (retry !== undefined && (!Number.isSafeInteger(retry) || Number(retry) < 0 || Number(retry) > MAX_AGENT_FLOW_COMMAND_RETRIES)) {
      addStepIssue(
        errors,
        context,
        `${failureCode}.retry.invalid`,
        "on_failure.retry",
        `${failureLabel} on_failure.retry must be an integer from 0 through ${MAX_AGENT_FLOW_COMMAND_RETRIES}.`
      );
    }
...
function validateArtifactOutputs(
  ...
  for (const context of contexts) {
    for (const { output, outputContext } of stepOutputContexts(context)) {
      ...
      if (outputKey === AGENT_FLOW_FINAL_SUMMARY_PATH) {
        addStepIssue(
          errors,
          outputContext,
          "workflow.artifact.output.reserved",
          outputField(outputContext.step),
          `Artifact "${AGENT_FLOW_FINAL_SUMMARY_PATH}" is reserved for the runtime's final pipeline summary.`
        );
        continue;
      }
  ...
  for (const context of contexts) {
    ...
    if (context.type === "session_request") {
      ...
      for (const field of ["inputs", "outputs"] as const) {
        ...
        if (field === "inputs" && values.length > MAX_AGENT_FLOW_SESSION_INPUTS) {
          addStepIssue(
            errors,
            context,
            "workflow.session_request.inputs.limit",
            field,
            `Session requests may declare at most ${MAX_AGENT_FLOW_SESSION_INPUTS} input artifacts.`
          );
        }
