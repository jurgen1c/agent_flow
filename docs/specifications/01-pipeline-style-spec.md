# Agent Flow Pipeline Style Spec

## 1. Purpose

Pipeline workflows are the simplest Agent Flow style. They execute a mostly linear sequence of steps where each step produces artifacts for later steps.
...
```bash
agent-flow resume <run-id> --outcome approve
```
...
```bash
agent-flow resume <run-id> --answer '{"environment":"staging"}'
```
