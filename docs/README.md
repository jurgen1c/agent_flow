...
5. Read [session provider boundaries](session-providers.md) before configuring
   local, frontier, pass-through Codex, named Codex-profile, MCP, or custom
   adapters.
...
- [Session provider boundaries](session-providers.md) defines typed provider
  registration, pass-through Codex and MCP behavior, live opt-in,
  request/response evidence, and failure behavior.
