...
describe("Agent Flow artifact transform steps", () => {
  test("simulates jira-ticket-spec with a fixture-derived Markdown artifact", () => {
    ...
    expect(result.artifactValues["ticket.md"]).toContain("# AM-24: Implement Agent Flow artifact transform steps for pipelines");
...
`);
  ...
  test("rejects oversized inputs through a bounded artifact read", async () => {
    ...
    store.writeArtifact({
      ...
      kind: "fixture",
      contentType: "application/json",
      content: Buffer.alloc(MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES + 1, 32)
    });
    ...
    expect(result.message).toContain(`exceeds the ${MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES}-byte read limit`);
  ...
  test("rejects an oversized stale replacement before checksumming it", async () => {
    ...
    fs.writeFileSync(path.join(temporaryRoot, input.storagePath), Buffer.alloc(MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES + 1, 32));
    const originalReadSync = fs.readSync;
    let bytesRead = 0;
    Object.defineProperty(fs, "readSync", {
      configurable: true,
...
`);
  ...
  test("rejects oversized transform fixture inputs during simulation", () => {
    const result = simulateAgentFlowWorkflow(transformWorkflow("jira_ticket_to_markdown"), {
      artifacts: { "ticket.json": "x".repeat(MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES + 1) }
    });

    expect(result.status).toBe("unresolved");
    expect(result.availableArtifacts).toEqual(["ticket.json"]);
    expect(result.unresolvedBranches[0]?.reason)
      .toContain(`exceeds the ${MAX_AGENT_FLOW_TRANSFORM_INPUT_BYTES}-byte transform input limit`);
  });
...
`);
    ...
    expect(result.status).toBe("completed");
    expect(result.artifactValues["encoded.bin"]).toEqual({ __agent_flow_binary__: "base64", data: "/wCA" });
...
`);
    ...
    const result = simulateAgentFlowWorkflow(workflow, {
      artifacts: {
        "source.txt": "ignored",
        "encoded.bin": { data: "/wCA", __agent_flow_binary__: "base64" }
      }
    }, registry);
...
`);
    ...
    const result = simulateAgentFlowWorkflow(workflow, {
      artifacts: { "encoded.bin": { __agent_flow_binary__: "base64", data: "not base64!" } }
    }, registry);
...
function temporaryRepo(): string {
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "agent-flow-transform-"));
