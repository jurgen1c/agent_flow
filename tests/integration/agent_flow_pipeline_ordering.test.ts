...
describe("Agent Flow pipeline ordering and conditions", () => {
...
`);
    ...
    await expect(executeAgentFlowCommandPipeline(store, "ambiguous-runtime-route", workflow))
      .rejects.toMatchObject({
        code: "AGENT_FLOW_STEP_AMBIGUOUS",
        message: 'Agent Flow workflow has multiple steps with ID "duplicate"; runtime routing is ambiguous.'
      });
...
function temporaryRepo(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agent-flow-ordering-"));
